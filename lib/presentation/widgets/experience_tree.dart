...
import 'package:subbu_portfolio/presentation/widgets/tree_painter.dart';
...
class _ExperienceBranchState extends State<ExperienceBranch> {
  ...
  Widget get experienctDetail => Padding(
  ...
  @override
  Widget build(BuildContext context) {
    bool isOdd = widget.index.isOdd;
    return CustomPaint(
      foregroundPainter: TreePainter(
          stalk: 0.1,
          veinsColor:
              AppColors.primaryColor.withOpacity(AppColors.primaryColorOpacity),
          outerJointColor:
              AppColors.primaryColor.withOpacity(AppColors.primaryColorOpacity),
            ...
            children: [
              ...
              durationIcon ??
                  Icon(Icons.access_time,
                      color: AppColors.accentColor2, size: 18),
            ],
          ),
          SpaceH8(),
          Row(
...
class RoleLeaf extends StatelessWidget {
  ...
  List<Widget> _buildRoles(
      {required List<String> roles, required BuildContext context}) {
    ThemeData theme = Theme.of(context);
